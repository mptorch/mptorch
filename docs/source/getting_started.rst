...
MPTorch is a C++/CUDA extension, so installing it compiles the kernels
against the PyTorch you have installed.
...
extension in a throw-away environment against a *freshly downloaded* PyTorch
that may not be the one you run, and the result fails to import with an
``undefined symbol`` error. With it, the build uses your installed PyTorch.

The CUDA kernels are built whenever ``torch.cuda.is_available()`` is true and
``CUDA_HOME`` is set. To force a CPU-only build, or a debug build:

.. code-block:: console

   $ USE_CUDA=0 pip3 install -e . --no-build-isolation
   $ DEBUG=1 pip3 install -e . --no-build-isolation

The test suite needs two extra packages, kept in a ``test`` extra:

.. code-block:: console

   $ pip3 install -e ".[test]" --no-build-isolation
   $ pytest tests/
...
arithmetic
   What happens *inside* a matrix product: which format each partial product
   and each running sum is rounded to. This is the part a plain quantizer
   cannot express, and the part MPTorch's kernels exist for.
