...
- **Custom arithmetic matrix multiplication.** The arithmetic *inside* the dot product -- each product, each addition to the running sum, or each fused multiply-add -- is rounded to a format you choose, on CPU and CUDA, with the full ``torch.matmul`` operand contract and gradients (:doc:`gemm`).
...
- **Reproducibility.** ``torch.manual_seed`` controls the stochastic rounding streams, and the deterministic modes are bit-identical between CPU and CUDA.
