"""Thirty seconds with MPTorch: round a tensor, then train a layer in that format."""
...
# A format is a value; a Quant is that format as a function on tensors.
e4m3 = BinaryK(8, 4)  # 8 bits: sign, 4 exponent, 3 mantissa
x = torch.randn(4)
print("x        ", x)
print("E4M3(x)  ", Quant(e4m3)(x))

# A layer takes one quantizer per signal. Anything left None stays float32.
