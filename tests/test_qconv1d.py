...
from mptorch.quant import QConv1d, QAffineFormats, binaryK_quantize
...
def test_qconv1d_tier1_exact(device, bias):
    ...
    q_layer = QConv1d(4, 8, kernel_size=3, padding=1, bias=bias, formats=formats, device=device, dtype=dtype)
...
def test_qconv1d_tier3_manual_baseline(device, dtype, bias):
    ...
    out_man = F.conv1d(qx, qw, qb, stride=q_layer.stride, padding=q_layer.padding, dilation=q_layer.dilation, groups=q_layer.groups)
    ...
    igrad_man = torch.nn.grad.conv1d_input(qx.shape, qw, q_igrad_out, stride=q_layer.stride, padding=q_layer.padding, dilation=q_layer.dilation, groups=q_layer.groups)
    wgrad_man = torch.nn.grad.conv1d_weight(qx, qw.shape, q_wgrad_out, stride=q_layer.stride, padding=q_layer.padding, dilation=q_layer.dilation, groups=q_layer.groups)
    if bias:
