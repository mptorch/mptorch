...
from mptorch.quant import QConv3d, QAffineFormats, binaryK_quantize
from mptorch.number import RoundMode

@pytest.mark.parametrize("device", available_devices)
@pytest.mark.parametrize("bias", [True, False])
def test_qconv3d_tier1_exact(device, bias):
    dtype = torch.float32
    ...
    vanilla = nn.Conv3d(4, 8, kernel_size=3, padding=1, bias=bias, device=device, dtype=dtype)
    q_layer = QConv3d(4, 8, kernel_size=3, padding=1, bias=bias, formats=formats, device=device, dtype=dtype)
...
@pytest.mark.parametrize("bias", [True, False])
def test_qconv3d_tier2_statistical(device, dtype, bias):
    def quant_fn(x):
...
@pytest.mark.parametrize("dtype", [torch.float32, torch.float16, torch.bfloat16])
@pytest.mark.parametrize("bias", [True, False])
def test_qconv3d_tier3_manual_baseline(device, dtype, bias):
    def quant_fn(x):
        return binaryK_quantize(x, K=8, P=4, rounding_mode=RoundMode.RNE)
    ...
    out_man = F.conv3d(qx, qw, qb, stride=q_layer.stride, padding=q_layer.padding, dilation=q_layer.dilation, groups=q_layer.groups)
    ...
    igrad_man = torch.nn.grad.conv3d_input(qx.shape, qw, q_igrad_out, stride=q_layer.stride, padding=q_layer.padding, dilation=q_layer.dilation, groups=q_layer.groups)
    wgrad_man = torch.nn.grad.conv3d_weight(qx, qw.shape, q_wgrad_out, stride=q_layer.stride, padding=q_layer.padding, dilation=q_layer.dilation, groups=q_layer.groups)
    if bias:
