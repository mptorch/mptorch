"""
``QConv3d`` against ``nn.Conv3d`` and against a hand-written baseline.
...
hand from the formats' own quantizers, ``F.conv3d`` for the forward and
``torch.nn.grad.conv3d_input`` / ``conv3d_weight`` for the backward, so
...
def test_qconv3d_tier1_exact(device, bias):
    """
    Tier 1: with identity quantizers on every tensor, ``QConv3d`` matches a
    vanilla ``nn.Conv3d`` in forward and in all three gradients.
    """
    dtype = torch.float32
...
def test_qconv3d_tier3_manual_baseline(device, dtype, bias):
    ...
    the same quantizers, ``F.conv3d`` forward and ``torch.nn.grad`` backward, so
    ...
    igrad_man = torch.nn.grad.conv3d_input(
        qx.shape,
        qw,
        q_igrad_out,
        stride=q_layer.stride,
    ...
    wgrad_man = torch.nn.grad.conv3d_weight(
        qx,
        qw.shape,
        q_wgrad_out,
        stride=q_layer.stride,
