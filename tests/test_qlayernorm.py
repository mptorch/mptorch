...
def test_qlayer_norm_custom(
    device, shape, normalized_shape, norm_formats, affine, bias
):
    layer = torch.nn.LayerNorm(normalized_shape, 1e-5, affine, bias, device)
    qlayer = QLayerNorm(normalized_shape, norm_formats, 1e-5, affine, bias).to(device)
    ...
    if bias and affine:
        torch.testing.assert_close(
            layer.bias.grad, qlayer.bias.grad, atol=1e-4, rtol=1e-4
        )
    if affine:
        torch.testing.assert_close(
...
def test_qlayer_norm_normal(device, shape, normalized_shape):
    ...
    qlayer = QLayerNorm(normalized_shape, QLayerNormFormats(), 1e-5, True, True).to(
        device
    )
    ...
    torch.testing.assert_close(
        layer.weight.grad, qlayer.weight.grad, atol=1e-8, rtol=1e-8
    )
