...
def test_cublas_config_for_format():
    assert (
        match_mac_format_with_cublas_types(23, 8, 10, 5, "RNE", True, True, False)
        is None
    )
    assert (
        match_mac_format_with_cublas_types(23, 8, 23, 8, "RNE", False, True, False)
        is None
    )
    assert match_mac_format_with_cublas_types(
        10, 5, 10, 5, "RNE", True, True, False
    ) == (mt.F16, mt.F16, ct.F16)
    assert match_mac_format_with_cublas_types(
        23, 8, 23, 8, "RNE", True, True, False
    ) == (mt.F32, mt.F32, ct.F32)
    assert match_mac_format_with_cublas_types(
        23, 8, 23, 8, "RNE", True, True, False, "f16"
    ) == (mt.F32, mt.F32, ct.F32_FAST_F16)
    assert match_mac_format_with_cublas_types(
        23, 8, 23, 8, "RNE", True, True, False, "bf16"
    ) == (mt.F32, mt.F32, ct.F32_FAST_BF16)
    assert match_mac_format_with_cublas_types(
        23, 8, 23, 8, "RNE", True, True, False, "tf32"
