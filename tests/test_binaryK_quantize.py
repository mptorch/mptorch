...
def test_binaryK_to_gfloat_nearest_even_signed(device, K):
    for P in range(2, K):
        ...
        gqx.apply_(lambda x: round_float(fi, x, rnd=RoundMode.TiesToEven))
        gqx = gqx.to(device)
        qx = binaryK_quantize(x, K, P, rounding="nearest_even")
