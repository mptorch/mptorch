...
def test_binaryK_stochastic(device, K, dtype, signedness):
    ...
    q_rd = binaryK_quantize(x_rand, K, P, rounding_mode=mptorch.number.RoundMode.RD, is_signed=signedness[1])
    q_ru = binaryK_quantize(x_rand, K, P, rounding_mode=mptorch.number.RoundMode.RU, is_signed=signedness[1])
    
    # Grid points
    x_grid = q_rd.clone()
    q_sr_grid = binaryK_quantize(x_grid, K, P, prng_bits=prng_bits, rounding_mode=mptorch.number.RoundMode.SR, is_signed=signedness[1])
    
    # Property 1: Exact representation identity (grid points shouldn't change)
    assert torch.all(q_sr_grid == x_grid), "Stochastic rounding altered an exactly representable grid point!"
    
    # Quantize the non-representable random points using Stochastic mode
    q_sr_rand = binaryK_quantize(x_rand, K, P, prng_bits=prng_bits, rounding_mode=mptorch.number.RoundMode.SR, is_signed=signedness[1])
    ...
    if not torch.all(valid_bounds):
        ...
        for i in idx[:5]:
            print(f"x={x_rand[i].item()}, RD={q_rd[i].item()}, RU={q_ru[i].item()}, SR={q_sr_rand[i].item()}")
        
    assert torch.all(valid_bounds), "Stochastic rounding produced a value outside the [RD, RU] bounds!"
    ...
    q_sr_large = binaryK_quantize(x_large, K, P, prng_bits=prng_bits, rounding_mode=mptorch.number.RoundMode.SR, is_signed=signedness[1])
    ...
    assert abs(mean_val - test_val) < tolerance, f"Stochastic rounding expectation biased! Expected {test_val}, got mean {mean_val}"
