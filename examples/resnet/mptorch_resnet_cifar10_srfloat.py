...
parser.add_argument(
    ...
    default=10,
    metavar="N",
    help="number of random bits (default: 10)",
)
...
parser.add_argument(
    ...
    default=20,
    help="must be one of the following values: 20, 32, 44, 56, 110, 1202",
)
...
i_format = FloatingPoint(exp=8, man=7, saturate=False)
b_format = FloatingPoint(exp=8, man=7, saturate=False)
...
quant_i = lambda x: qpt.float_quantize(
    x, exp=i_format.exp, man=i_format.man, rounding=rounding, saturate=False
)
quant_b = lambda x: qpt.float_quantize(
    x, exp=b_format.exp, man=b_format.man, rounding=rounding, saturate=False
)
