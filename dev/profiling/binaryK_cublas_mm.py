...
import torch
from mptorch.quant import float_mm, binaryK_quantize, cublas_acceleration
import argparse
...
if torch.cuda.is_available():
    ...
    print(
        "Benchmarking binaryK_quantize followed by float_mm without cublas acceleration."
    )
    torch.manual_seed(0)
    ...
    with profile(activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA]) as prof:
        with record_function("quant_binary8_and_float_mm"):
            binaryK_quantize(a, 8, 1, "SR", "saturate_finite", True)
            binaryK_quantize(b, 8, 1, "SR", "saturate_finite", True)
            float_mm(
                a,
                b,
    ...
    print(
        "Benchmarking binaryK_quantize followed by float_mm with cublas acceleration on."
    )
    torch.manual_seed(0)
    torch.cuda.manual_seed(0)
    a = torch.rand(m, k).cuda()
