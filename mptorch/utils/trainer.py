...
def trainer(net, train_loader, test_loader, num_epochs, lr, batch_size, 
            loss=nn.CrossEntropyLoss(reduction='mean'), 
            loss_scaling=1, optimizer=None, 
            device=try_gpu(), scheduler=None):
    ...
    for epoch in range(num_epochs):
        ...
        for X, y in train_loader:
            ...
            tq.set_postfix(train_acc='{:.5f}'.format(train_acc / train_size), 
        ...
        if scheduler is not None:
            scheduler.step()
        print(f'loss {train_loss / train_size:.3f}, train acc {train_acc / train_size:.3f}, test acc {test_acc:.3f}')