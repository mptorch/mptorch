...
// binaryK formats under AccumulateAlgorithm::TREE (pairwise summation within a
// block; common/gemm_accumulate.h), in binary32: the float32, float16 and
// bfloat16 kernels of custom_matmul_binaryK. There is no fused twin: a tree
...
// kernels (seven round modes, with and without an accumulate format). There is
// no *_f64 twin yet: the three algorithms are binary32 only until
// dev/continuation_plan.md's phase G.
...
namespace mptorch::gemm_cuda
{
    ...
    template void CudaBackend::launch_accumulated_as<float, AccumulateAlgorithm::TREE, BinaryKSplitArgs>(
