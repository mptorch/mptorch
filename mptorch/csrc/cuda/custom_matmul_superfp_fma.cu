    ...
    int64_t fma_prng_bits)
{
    ...
    const mptorch::GemmDtype dt = mptorch::gemm_dtype_of(a_c, b_c, "custom_matmul_superfp_fma");
    ...
    mptorch::dispatch_round_mode(rm, [&](auto rm_c)
    {
        ...
        if (fma_quant)
        {
            SuperfpAdder<RM> add{static_cast<int>(fma_man_bits), static_cast<int>(fma_exp_bits),
    ...
    int64_t accumulate_algorithm, int64_t round_mode, int64_t saturation_mode, int64_t fma_prng_bits)
{
    ...
    const mptorch::GemmDtype dt = mptorch::gemm_dtype_of(a_c, b_c, "custom_matmul_superfp_fma_mixed");
    const void *p_a = a_c.data_ptr();
    const void *p_b = b_c.data_ptr();
    void *p_c = c.data_ptr();
    ...
    mptorch::dispatch_round_mode(rm, [&](auto rm_c)
    {
        ...
        {
            SuperfpAdder<RM> add{static_cast<int>(fma_man_bits[i]), static_cast<int>(fma_exp_bits[i]),
                                 static_cast<int>(fma_normal_binades[i]), static_cast<int>(fma_bias[i]),
                                 fma_is_signed, sat, static_cast<int>(fma_prng_bits)};
            pal.slots[i] = Mac{add};
        }
