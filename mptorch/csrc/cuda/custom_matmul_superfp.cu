...
// superfp formats, split mac, in binary32: the float32, float16 and bfloat16
// kernels of custom_matmul_superfp and its mixed-format twin.
// custom_matmul_superfp_f64.cu holds the same two ops' binary64 kernels.
//
...
namespace mptorch::gemm_cuda
{
    ...
    template void CudaBackend::launch_as<float, SuperfpSplitArgs>(
        const GemmShape &, const SuperfpSplitArgs &, const CudaBackend::LaunchContext &);
    template void CudaBackend::launch_as<float, SuperfpSplitMixedArgs>(
