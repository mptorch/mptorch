...
// construction (gemm_policy.h) for the GEMM, once per tensor for the
// elementwise quantizers -- rather than re-deriving its constants from
// man_bits/exp_bits/bias on every call.
//
// That derivation used to be a second overload of each function, taking the
// format parameters directly. Nothing in the extension has called it since C4
// and E3, so it now lives in dev/benchmarks/reference_casts.h, where the
// exhaustive sweeps check these bodies against it over all 2^32 float inputs
// (dev/gemm_roadmap.md, finding H2). The two are independent derivations of
// the same format on purpose: change one and change the other, or the sweeps
// will say so.

// The constants those casts read. Round-mode-agnostic -- every field uses the
// same formula whichever cast reads it, so one struct serves all of them and a
// mode simply leaves the fields it does not need alone. See bit_helper.h's
// "Precomputed-parameter forms" note for what is worth precomputing and what
// is not.
//
// Flat by design -- see dev/gemm_roadmap.md item 6, and its lesson "Flat
// kernel-argument structs": a nested aggregate replicated across the GEMM's
// instantiations took cicc from 18 s to ten minutes. A base class is a nested
// aggregate too, which is why the fast path's fields are not one: the primary
// template below is every carrier's, and the binary32 specialization after it
// repeats its fields, in the same order, before adding the fast path's. The
// binary32 layout is therefore the one it always was, and a binary64 struct
// carries no fields it cannot use.
template <class T>
...
// The carrier defaults to binary32, which is what every caller that names no
// carrier has always built.
                                                    ...
                                                    const BinaryKParamsT<T> &p)
{
    ...
    // `if constexpr`. Put the test inside it too and nvcc lowers the `&&`
    // with its operands the other way round in the SR arm below: the same
    // instructions, but no longer the same kernel.
...
// `1 - p.min_exp` rather than stored separately.
template <class T>
...
// origin_float >= 0. The subnormal branch's exp_diff is data-dependent
// (varies per call), so round_bitwise_up(target, exp_diff) there is left
// as-is -- only the normal branch's man_bits-derived round precomputes.
template <class T>
CUDA_HOST_DEVICE_INLINE T cast_absolute_up(T origin_float, SubnormalsMode subnormals, const BinaryKParamsT<T> &p)
{
    using F = FloatTraits<T>;
    using word_t = typename F::word_t;
    word_t target, quantize_bits;
...
// The _down half of that helper -- see cast_absolute_up above.
template <class T>
CUDA_HOST_DEVICE_INLINE T cast_absolute_down(T origin_float, SubnormalsMode subnormals, const BinaryKParamsT<T> &p)
{
    using F = FloatTraits<T>;
    using word_t = typename F::word_t;
    word_t target, quantize_bits;
...
// Stochastic rounding. Unlike the deterministic modes this still takes a
// per-call random value (rand_prob) and rand_bits (a Multiplier/Adder-fixed
// but role-specific width, not folded into BinaryKParamsT) -- only the
// round-to-nearest-even bit-twiddling constants precompute away. rand_prob is
// a word of the carrier: its MAN_BITS low bits are the draw.
template <class T>
                                                  ...
                                                  const BinaryKParamsT<T> &p)
{
    ...
    if (subnormal && (subnormals == SubnormalsMode::SUBNORMALS))
    {
        ...
        shift_float = reinterpret_cast<const T &>(shift_bits);
        val = origin_float + shift_float;
        target = reinterpret_cast<const word_t &>(val);
