...
// every call, is a template on the carrier (bit_helper.h's FloatTraits), and
// the format-taking spelling that the exhaustive sweeps check them against
// lives in dev/benchmarks/reference_casts.h (finding H2).
...
// normal_cutoff)), or underflow (< supernormal_cutoff). Neither depends on the
// carrier, so this is not a template.
...
// The supernormal region spans (2^exp_bits - normal_binades) * 2^man_bits
// binades, which is past `int` once exp_bits + man_bits reaches 31 -- exp_bits
// 9 with binary32's 23 mantissa bits already does it, and binary64's 52 always
// would. So the span is taken in 64 bits, and a cutoff below `int`'s range is
// clamped to one above INT_MIN (so that `supernormal_cutoff - 1` stays
// representable). The clamp moves no answer: every exponent a carrier's word
// can hold is above either value, so the underflow region is empty both ways.
// Every cutoff `int` could hold before is unchanged; the ones it could not were
// wrapping.
...
// same reason: the primary template is every carrier's, and the binary32
// specialization repeats its fields before adding the fast path's (see the
// note above BinaryKParamsT, and dev/gemm_roadmap.md item 6).
//
...
struct SuperfpParamsT
{
    ...
    // format carries for its floor stay out of this struct -- which is where
    // the superfp split-mac's registers are (finding G10).
...
// The carrier defaults to binary32, as make_binaryK_params' does.
                                                              ...
                                                              int bias, SaturationMode saturation_mode)
{
    ...
    if constexpr (F::HAS_FAST_CAST)
    {
        ...
        // With the underflow region out of binary32's reach there is no floor to
        // lift a rounded magnitude to, so fast_super_min is zero and the fmax
        // leaves the rounding alone -- which is what "nothing underflows" means.
        //
        // fast_super_half is still a real threshold there, because the *rounding*
        // has one binary32 does not: the nearest power of two to a float32
        // subnormal below 2^-127 is zero, so the supernormal arm returns a
        // magnitude of zero for those and for nothing else (the word test is
        // ab < 0x00400000, and ab == 0x00400000 ties up to 2^-126). Naming that
        // boundary here -- the largest float32 below 2^-127 -- rather than 0 is
        // what keeps a zero out of the fast path's copysign, and with it the
        // sign-clear that would otherwise have to run on every value (T3).
...
// MPTORCH_FAST_CAST gate. See bit_helper.h. binary32's only, like that twin.
...
CUDA_HOST_DEVICE_INLINE float cast_superfp_rne_fast(float origin_float, const SuperfpParams &p)
{
    ...
    // it -- P3109's zero is unsigned, and so is superfp's (T2). Signing first
    // costs nothing: it is the same copysign, one select earlier, and it is
    // what lets this path drop the sign-clear it used to return through (T3).
    ...
    // input itself -- exact bit pattern, payload and all, where the copysign
    // this replaced had to carry it back on.
...
CUDA_HOST_DEVICE_INLINE T cast_superfp_nearest_even(T origin_float, bool is_signed, const SuperfpParamsT<T> &p)
{
    ...
    // why the test stands outside the `if constexpr` too -- and bit_helper.h
    // for why this is the one fast path the host does not take.
    if (p.fast_rne)
    {
        if constexpr (F::HAS_FAST_CAST)
    ...
    else if (supernormal)
    {
        ...
        // the sign. Two instructions on a word already in a register; the rest
        // of the arm is unchanged (T3).
    ...
    else if (underflow)
    {
        ...
        // anything strictly below always flushes to zero -- +0.0, since
        // superfp has no -0.0 encoding any more than P3109 does (T2).
...
CUDA_HOST_DEVICE_INLINE T cast_superfp_up(T origin_float, bool is_signed, const SuperfpParamsT<T> &p)
{
    ...
    // NaN comparisons are always false, so a NaN takes the negating arm below.
    // That used to need a non-finite guard here, because `-x` on the device
    // may canonicalize a NaN's payload and sign; negating on the word instead
    // carries it through the helper's own NaN/inf arm untouched, which is what
    // the guard was for. As in cast_binaryK_up, the helper gives back a
    // magnitude, so the same negation is the one way left to a -0.0.
    if (origin_float >= 0)
...
// constants precompute away -- and rand_prob is a word of the carrier.
template <class T>
                                                  ...
                                                  int rand_bits, bool is_signed, const SuperfpParamsT<T> &p)
{
    ...
    else
    {
        ...
        // puts that at or above the arm's floor, and `y` is nonzero.
        if (p.fast_rne)
        {
            if constexpr (F::HAS_FAST_CAST)
            {
