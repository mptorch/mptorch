...
namespace mptorch::gemm_cpu
{
  ...
  template void CpuBackend::launch_accumulated_as<float, AccumulateAlgorithm::BLOCK, BinaryKSplitArgs>(
    const GemmShape &, const AccumulateArgs<BinaryKSplitArgs> &, const CpuBackend::LaunchContext &);
  template void CpuBackend::launch_accumulated_as<float, AccumulateAlgorithm::BLOCK, BinaryKFusedArgs>(
