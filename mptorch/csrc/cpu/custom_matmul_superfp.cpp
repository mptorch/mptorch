    ...
    int64_t mul_prng_bits, int64_t acc_prng_bits)
{
  ...
  mptorch::dispatch_round_mode(rm, [&](auto rm_c)
  {
    ...
    auto make_mul = [&](int64_t i) {
      return SuperfpMultiplier<RM>{static_cast<int>(mul_man_bits[i]), static_cast<int>(mul_exp_bits[i]),
    ...
    if (accumulate_quant)
    {
      ...
      {
        SuperfpAdder<RM> add{static_cast<int>(acc_man_bits[i]), static_cast<int>(acc_exp_bits[i]),
