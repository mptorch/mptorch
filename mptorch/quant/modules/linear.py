...
class CustomArithLinear(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.needs_input_grad[0]:
            bwd_igrad_math = formats.bwd_igrad_math if formats.bwd_igrad_math else CustomArithLinear._default_bwd_igrad
            grad_input = bwd_igrad_math(q_igrad_output, q_weight)
            
        if ctx.needs_input_grad[1]:
            bwd_wgrad_math = formats.bwd_wgrad_math if formats.bwd_wgrad_math else CustomArithLinear._default_bwd_wgrad
