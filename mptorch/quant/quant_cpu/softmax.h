...
void softmax_forward(const float *a_array, float *o_array, const DimStrides& strides, Qexp quant_exp, Qoff quant_off, Qacc quant_acc)
{
...
void softmax_lse_forward(const float *a_array, float *o_array, const DimStrides& strides, Qoff quant_off, Qlse quant_lse)
{
  for (int i = 0; i < strides.outer_size * strides.inner_size; ++i) {
    int outer_idx = i / strides.inner_size;
    int inner_idx = i % strides.inner_size;
...
void softmax_backward(const float *a_array, const float *g_array, float *o_array, const DimStrides& strides, Qadd quant_add, Qmul quant_mul)
{
  for (int i = 0; i < strides.outer_size * strides.inner_size; ++i) {
    int outer_idx = i / strides.inner_size;
    int inner_idx = i % strides.inner_size;
