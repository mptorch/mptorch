                           ...
                           int bias, bool saturate, SubnormalsMode subnormals)
{
  ...
  else
  {
    ...
    if (subnormal && (subnormals == SubnormalsMode::SUBNORMALS))
    {
      ...
      int not_uflow =
          exp_diff > -1 || ((exp_diff == -1) && ((target << 9) > 0));
    ...
    else
    {
      ...
      quantize_bits = clip_normal_range_exponent(
          exp_bits, man_bits, bias, target, quantize_bits, saturate,
  ...
  else
  {
    ...
    if (subnormal && (subnormals == SubnormalsMode::SUBNORMALS))
    {
      ...
      int not_uflow =
          exp_diff > -1 || ((exp_diff == -1) && ((target << 9) > 0));
      quantize_bits =
          not_uflow * round_bitwise_up(target, exp_diff < 0 ? 0 : exp_diff);
                 ...
                 bool saturate, SubnormalsMode subnormals)
{
  if (origin_float >= 0.0f)
    return cast_absolute_up(origin_float, man_bits, exp_bits, bias, saturate,
                            subnormals);
  else
...
{
  if (origin_float >= 0.0f)
    return cast_absolute_down(origin_float, man_bits, exp_bits, bias, saturate,
                              subnormals);
  else
    return -cast_absolute_up(-origin_float, man_bits, exp_bits, bias, saturate,
                             subnormals);
...
{
  if (origin_float >= 0.0f)
    return cast_fp_down(origin_float, man_bits, exp_bits, bias, saturate,
                        subnormals);
  else
    return cast_fp_up(origin_float, man_bits, exp_bits, bias, saturate,
                      subnormals);
}
