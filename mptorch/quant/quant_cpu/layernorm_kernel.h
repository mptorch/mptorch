...
void layernorm_forward(const float *a_array, const float *w_array,
                       const float *b_array, float *o_array, float *m_array,
...
void layernorm_backward(const float *a_array, const float *g_array,
                        ...
                        float *o_array, float *grad_gamma, float *grad_beta,
                        const DimSizes &sizes, Qacc quant_acc, Qmul quant_mul,
                        Qdiv quant_div) {
  for (int i = 0; i < sizes.outer * sizes.inner; i++) {
    int b = i / sizes.inner;
    int t = i % sizes.inner;
    ...
    for (int k = 0; k < sizes.channel; k++) {
      ...
      // calculate the grad_beta (gradient of biases) and grad_gamma (gradient
      // of weights)
