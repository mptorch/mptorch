...
unsigned int round_bitwise(unsigned int target, int man_bits, Mode rounding) {
    if (man_bits == 23){
        // Probably better just to return the target as it will do 32bit -> 32bit quantization
        return target;
    }
...
Tensor get_max_entry(Tensor a, int dim) {
    ...
    if (dim == -1) {
        ...
        Tensor max_transpose = std::get<0>(input_view.abs().max(1, true)).expand_as(input_view).view_as(
                input_transpose);
...
float float_quantize(float origin_float, int man_bits, int exp_bits, Mode rounding) {
    unsigned int target, quantize_bits;
...
void tvm_gemm(float *a, float *b, float *c, int M, int K, int N,
              int man_bits, int exp_bits) {
    for (int64_t i = 0; i < M; ++i)
        for (int64_t j = 0; j < N; ++j)
            for (int64_t k = 0; k < K; ++k) {
                c[i * N + j] = float_quantize(c[i * N + j] + float_quantize(a[i * K + k] * b[k * N + j],
                                                                            man_bits, exp_bits, rNearest), man_bits,
                                              exp_bits, rNearest);
            }
}
...
void float_quantize_gemm(Tensor a, Tensor b, Tensor c, int M, int N, int K,
                         int man_bits, int exp_bits) {
    tvm_gemm(a.data_ptr<float>(), b.data_ptr<float>(), c.data_ptr<float>(),
...
m.def("float_quantize_gemm", &float_quantize_gemm, "Low-Bitwidth GEMM (CPU)");
}
