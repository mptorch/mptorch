...
// stochastic rounding
uint32_t round_bitwise_stochastic(uint32_t target, uint32_t rand_prob,
                                  int man_bits) {
...
uint32_t clip_exponent(int exp_bits, int man_bits, uint32_t old_num,
  ...
  if (quantized_exponent_store > max_exponent_store) {
    if (saturate) {
      uint32_t max_man = (uint32_t)-1 << 9 >> 9 >> (23 - man_bits)
                                                       << (23 - man_bits);
...
uint32_t clip_normal_range_exponent(int exp_bits, int man_bits, int bias,
  ...
  uint32_t max_man = ((0x007FFFFF >> (23 - man_bits)) - 1 + finite)
                     << (23 - man_bits);
  uint32_t max_num = ((uint32_t)max_exponent_store << 23) | max_man;
  ...
  // handle underflow
    ...
    uint32_t offset = (quantized_exponent_store == (min_exponent_store - 1)) &&
                      ((old_num << 9 >> 9) > (1 << 22));
