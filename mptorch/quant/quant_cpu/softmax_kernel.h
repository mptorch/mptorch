...
void softmax_forward(const float *a_array, float *o_array,
...
void softmax_lse_forward(const float *a_array, float *o_array,
...
void softmax_backward(const float *a_array, const float *g_array,
                      float *o_array, const DimSizes &sizes, Qadd quant_add,
                      Qmul quant_mul) {
  for (int i = 0; i < sizes.outer * sizes.inner; ++i) {
    int outer_idx = i / sizes.inner;
    int inner_idx = i % sizes.inner;
