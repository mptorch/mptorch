...
Tensor float_quantize_nearest_even(Tensor a, int man_bits, int exp_bits,
...
Tensor float_quantize_nearest_away(Tensor a, int man_bits, int exp_bits,
                                   bool subnormals, bool saturate) {
  CHECK_INPUT(a);
...
Tensor float_quantize_up(Tensor a, int man_bits, int exp_bits, bool subnormals,
                         bool saturate) {
  CHECK_INPUT(a);
  return float_quantize_up_cuda(a, man_bits, exp_bits, subnormals, saturate);
}

Tensor float_quantize_down(Tensor a, int man_bits, int exp_bits,
                           bool subnormals, bool saturate) {
  CHECK_INPUT(a);
  return float_quantize_down_cuda(a, man_bits, exp_bits, subnormals, saturate);
}

Tensor float_quantize_zero(Tensor a, int man_bits, int exp_bits,
                           bool subnormals, bool saturate) {
  CHECK_INPUT(a);
  return float_quantize_zero_cuda(a, man_bits, exp_bits, subnormals, saturate);
}

Tensor superfp_quantize_nearest(Tensor a, int man_bits, int exp_bits,
                                int binades_l, int binades_u, bool saturate) {
  CHECK_INPUT(a);
  return superfp_quantize_nearest_cuda(a, man_bits, exp_bits, binades_l,
                                       binades_u, saturate);
}
...
Tensor binaryK_quantize_nearest_even(Tensor a, int K, int P, bool is_signed,
  ...
  CHECK_INPUT(a);
  return binaryK_quantize_nearest_even_cuda(a, K, P, is_signed, saturation_mode,
                                            bias);
}
...
Tensor binaryK_quantize_nearest_away(Tensor a, int K, int P, bool is_signed,
                                     SaturationMode saturation_mode, int bias) {
  CHECK_INPUT(a);
  return binaryK_quantize_nearest_away_cuda(a, K, P, is_signed, saturation_mode,
                                            bias);
}
...
Tensor binaryK_quantize_up(Tensor a, int K, int P, bool is_signed,
                           SaturationMode saturation_mode, int bias) {
  CHECK_INPUT(a);
  return binaryK_quantize_up_cuda(a, K, P, is_signed, saturation_mode, bias);
}

Tensor binaryK_quantize_down(Tensor a, int K, int P, bool is_signed,
                             SaturationMode saturation_mode, int bias) {
  CHECK_INPUT(a);
  return binaryK_quantize_down_cuda(a, K, P, is_signed, saturation_mode, bias);
}

Tensor binaryK_quantize_zero(Tensor a, int K, int P, bool is_signed,
                             SaturationMode saturation_mode, int bias) {
  CHECK_INPUT(a);
  return binaryK_quantize_zero_cuda(a, K, P, is_signed, saturation_mode, bias);
}
...
Tensor binaryK_quantize_stochastic(Tensor a, int K, int P, int prng_bits,
                                   ...
                                   SaturationMode saturation_mode, int bias) {
  CHECK_INPUT(a);
  return binaryK_quantize_stochastic_cuda(a, K, P, prng_bits, is_signed,
                                          saturation_mode, bias);
}
...
Tensor float_quantize_stochastic(Tensor a, int man_bits, int exp_bits,
                                 int prng_bits, bool subnormals,
                                 bool saturate) {
  CHECK_INPUT(a);
  return float_quantize_stochastic_cuda(a, man_bits, exp_bits, prng_bits,
                                        subnormals, saturate);
}
...
void float_quantize_nearest_bmm(Tensor a, Tensor b, Tensor c, int M, int N,
  ...
  CHECK_INPUT(b);
  CHECK_INPUT(c);
  float_quantize_nearest_bmm_cuda(a, b, c, M, N, K, man_add, exp_add, man_mul,
                                  exp_mul, subnormals, saturate, compensated);
  return;
}
...
void superfp_quantize_nearest_mm(Tensor a, Tensor b, Tensor c, int M, int N,
                                 int K, int man_add, int exp_add, int man_mul,
                                 int exp_mul, int binades_add_l,
                                 int binades_add_u, int binades_mul_l,
...
void superfp_quantize_nearest_bmm(Tensor a, Tensor b, Tensor c, int M, int N,
  ...
  CHECK_INPUT(b);
  CHECK_INPUT(c);
  superfp_quantize_nearest_bmm_cuda(a, b, c, M, N, K, man_add, exp_add, man_mul,
                                    exp_mul, binades_add_l, binades_add_u,
                                    binades_mul_l, binades_mul_u, saturate);
  return;
}
...
void superfp_quantize_nearest_bmm_fma(Tensor a, Tensor b, Tensor c, int M,
  ...
  CHECK_INPUT(b);
  CHECK_INPUT(c);
  superfp_quantize_nearest_bmm_fma_cuda(a, b, c, M, N, K, man_fma, exp_fma,
                                        binades_fma_l, binades_fma_u, saturate);
  return;
}
...
void float_quantize_stochastic_mm(Tensor a, Tensor b, Tensor c, int M, int N,
                                  ...
                                  int rbits_add, int man_mul, int exp_mul,
                                  int rbits_mul, bool subnormals,
                                  bool saturate) {
  CHECK_INPUT(a);
  CHECK_INPUT(b);
  CHECK_INPUT(c);
...
void float_quantize_stochastic_bmm(Tensor a, Tensor b, Tensor c, int M, int N,
  ...
  CHECK_INPUT(b);
  CHECK_INPUT(c);
  float_quantize_stochastic_bmm_cuda(a, b, c, M, N, K, man_add, exp_add,
                                     rbits_add, man_mul, exp_mul, rbits_mul,
                                     subnormals, saturate);
  return;
}
...
void fixed_point_quantize_nearest_bmm(Tensor a, Tensor b, Tensor c, int M,
                                      int N, int K, int wl_add, int fl_add,
                                      int wl_mul, int fl_mul, bool symmetric) {
  CHECK_INPUT(a);
  CHECK_INPUT(b);
  CHECK_INPUT(c);
  fixed_point_quantize_nearest_bmm_cuda(a, b, c, M, N, K, wl_add, fl_add,
...
void floating_point_bmm_cublas(Tensor a, Tensor b, Tensor c, int M, int N,
                               int K, CUBLASMatrixType AB_type,
                               CUBLASMatrixType C_type,
                               CUBLASComputeType compute_type, bool pedantic) {
  CHECK_INPUT(a);
  CHECK_INPUT(b);
  CHECK_INPUT(c);
...
void float_quantize_layernorm_forward(Tensor input, Tensor weight, Tensor bias,
                                      ...
                                      int man_acc, int exp_acc, int man_mul,
                                      int exp_mul, int man_div, int exp_div,
  ...
  CHECK_INPUT(mean);
  CHECK_INPUT(rstd);
  float_quantize_nearest_layernorm_forward_cuda(
      input, weight, bias, output, mean, rstd, eps, dims, man_acc, exp_acc,
      man_mul, exp_mul, man_div, exp_div, man_sqrt, exp_sqrt, subnormals,
      saturate);
}

void float_quantize_layernorm_backward(
    Tensor input, Tensor grad_output, Tensor weight, Tensor bias, Tensor mean,
    Tensor rstd, Tensor grad_input, Tensor grad_gamma, Tensor grad_beta,
    std::vector<int> &dims, int man_acc, int exp_acc, int man_mul, int exp_mul,
    int man_div, int exp_div, bool subnormals, bool saturate) {
  CHECK_INPUT(input);
  ...
  float_quantize_nearest_layernorm_backward_cuda(
      input, grad_output, weight, bias, mean, rstd, grad_input, grad_gamma,
      grad_beta, dims, man_acc, exp_acc, man_mul, exp_mul, man_div, exp_div,
      subnormals, saturate);
}

void superfp_quantize_layernorm_forward(
    Tensor input, Tensor weight, Tensor bias, Tensor output, Tensor mean,
    Tensor rstd, float eps, std::vector<int> &dims, int man_acc, int exp_acc,
  ...
  CHECK_INPUT(mean);
  CHECK_INPUT(rstd);
  superfp_quantize_nearest_layernorm_forward_cuda(
      input, weight, bias, output, mean, rstd, eps, dims, man_acc, exp_acc,
      binades_acc_l, binades_acc_u, man_mul, exp_mul, binades_mul_l,
      binades_mul_u, man_div, exp_div, binades_div_l, binades_div_u, man_sqrt,
      exp_sqrt, binades_sqrt_l, binades_sqrt_u, saturate);
}

void superfp_quantize_layernorm_backward(
    Tensor input, Tensor grad_output, Tensor weight, Tensor bias, Tensor mean,
    Tensor rstd, Tensor grad_input, Tensor grad_gamma, Tensor grad_beta,
    std::vector<int> &dims, int man_acc, int exp_acc, int binades_acc_l,
  ...
  CHECK_INPUT(grad_gamma);
  CHECK_INPUT(grad_beta);
  superfp_quantize_nearest_layernorm_backward_cuda(
      input, grad_output, weight, bias, mean, rstd, grad_input, grad_gamma,
      grad_beta, dims, man_acc, exp_acc, binades_acc_l, binades_acc_u, man_mul,
      exp_mul, binades_mul_l, binades_mul_u, man_div, exp_div, binades_div_l,
      binades_div_u, saturate);
}
...
void float_quantize_nearest_softmax_forward(Tensor a, Tensor o, int dim,
  ...
  CHECK_INPUT(a);
  CHECK_INPUT(o);
  float_quantize_nearest_softmax_forward_cuda(a, o, dim, man_exp, exp_exp,
                                              man_off, exp_off, man_acc,
                                              exp_acc, subnormals, saturate);
}
...
void float_quantize_nearest_softmax_lse_forward(Tensor a, Tensor o, int dim,
  ...
  float_quantize_nearest_softmax_lse_forward_cuda(
      a, o, dim, man_off, exp_off, man_lse, exp_lse, subnormals, saturate);
}

void float_quantize_nearest_softmax_backward(Tensor a, Tensor g, Tensor o,
                                             int dim, int man_add, int exp_add,
  ...
  float_quantize_nearest_softmax_backward_cuda(
      a, g, o, dim, man_add, exp_add, man_mul, exp_mul, subnormals, saturate);
}

void superfp_quantize_nearest_softmax_forward(
    Tensor a, Tensor o, int dim, int man_exp, int exp_exp, int binades_exp_l,
    int binades_exp_u, int man_off, int exp_off, int binades_off_l,
    int binades_off_u, int man_acc, int exp_acc, int binades_acc_l,
    int binades_acc_u, bool saturate) {
  CHECK_INPUT(a);
  CHECK_INPUT(o);
  superfp_quantize_nearest_softmax_forward_cuda(
      a, o, dim, man_exp, exp_exp, binades_exp_l, binades_exp_u, man_off,
      exp_off, binades_off_l, binades_off_u, man_acc, exp_acc, binades_acc_l,
      binades_acc_u, saturate);
}

void superfp_quantize_nearest_softmax_lse_forward(
    Tensor a, Tensor o, int dim, int man_off, int exp_off, int binades_off_l,
    int binades_off_u, int man_lse, int exp_lse, int binades_lse_l,
    int binades_lse_u, bool saturate) {
  CHECK_INPUT(a);
  CHECK_INPUT(o);
  superfp_quantize_nearest_softmax_lse_forward_cuda(
      a, o, dim, man_off, exp_off, binades_off_l, binades_off_u, man_lse,
      exp_lse, binades_lse_l, binades_lse_u, saturate);
}

void superfp_quantize_nearest_softmax_backward(
    Tensor a, Tensor g, Tensor o, int dim, int man_add, int exp_add,
    int binades_add_l, int binades_add_u, int man_mul, int exp_mul,
    int binades_mul_l, int binades_mul_u, bool saturate) {
  CHECK_INPUT(a);
  CHECK_INPUT(g);
  CHECK_INPUT(o);
  superfp_quantize_nearest_softmax_backward_cuda(
      a, g, o, dim, man_add, exp_add, binades_add_l, binades_add_u, man_mul,
      exp_mul, binades_mul_l, binades_mul_u, saturate);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("superfp_quantize_nearest", &superfp_quantize_nearest,
        "Low-Bitwidth Super Normal Floating Point Number Nearest Neighbor "
        "Quantization "
        "(CUDA)");

  m.def("binaryK_quantize_nearest_even", &binaryK_quantize_nearest_even,
        "Custom-precision P3109 Floating-Point Quantization with Nearest "
        "Rounding Ties To Even (GPU)");

  m.def("binaryK_quantize_nearest_away", &binaryK_quantize_nearest_away,
        "Custom-precision P3109 Floating-Point Quantization with Nearest "
        "Rounding Ties To Away (GPU)");

  m.def("binaryK_quantize_up", &binaryK_quantize_up,
        "Custom-precision P3109 Floating-Point Quantization with Rounding "
        "Towards Positive (GPU)");

  m.def("binaryK_quantize_down", &binaryK_quantize_down,
        "Custom-precision P3109 Floating-Point Quantization with Rounding "
        "Towards Negative (GPU)");

  m.def("binaryK_quantize_zero", &binaryK_quantize_zero,
        "Custom-precision P3109 Floating-Point Quantization with Rounding "
        "Towards Zero (GPU)");

  m.def("binaryK_quantize_stochastic", &binaryK_quantize_stochastic,
        "Custom-precision P3109 Floating-Point Quantization with Stochastic "
        "Rounding (GPU)");
  ...
  m.def("superfp_quantize_nearest_mm", &superfp_quantize_nearest_mm,
        "Low-Bitwidth SuperNormal Floating Point Number GEMM Quantization "
        "(CUDA)");
  ...
  m.def("create_cublas_handle", &create_cublas_handle,
        "Creates a new cuBLAS handle");
  m.def("delete_cublas_handle", &delete_cublas_handle,
        "Deletes the current cuBLAS handle");
  m.def("floating_point_mm_cublas", &floating_point_mm_cublas,
        "cuBLAS accelerated matrix multiply, using the specified precision and "
        "compute mode (CUDA)");
  m.def("floating_point_bmm_cublas", &floating_point_bmm_cublas,
        "cuBLAS accelerated batched matrix multiply, using the specified "
        "precision "
