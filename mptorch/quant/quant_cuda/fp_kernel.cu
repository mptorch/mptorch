                   ...
                   bool compensated)
{
  ...
  if (compensated)
  {
    mm_kahan_impl<SHMEM_SIZE><<<block_dim, thread_dim>>>(
        ...
        [man_mul, exp_mul, bias_mul, subnormals, saturate] __device__(float x)
                       ...
                       bool compensated)
{
  ...
  if (compensated)
  {
    mm_kahan_fma_impl<SHMEM_SIZE><<<block_dim, thread_dim>>>(
        a, b, c, M, K, N,
        [man_fma, exp_fma, bias_fma, subnormals, saturate] __device__(float x)
                      ...
                      bool subnormals, bool saturate)
{
  ...
  mm_sr_impl<SHMEM_SIZE, uint32_t><<<block_dim, thread_dim>>>(
      ...
      state,
      M, K, N,
      [man_add, exp_add, bias_add, rbits_add, subnormals, saturate] __device__(float x, uint32_t rnd)
      { return cast_fp_stochastic(x, rnd, rbits_add, man_add, exp_add, bias_add, subnormals, saturate); },
      [man_mul, exp_mul, bias_mul, rbits_mul, subnormals, saturate] __device__(float x, uint32_t rnd)
      { return cast_fp_stochastic(x, rnd, rbits_mul, man_mul, exp_mul, bias_mul, subnormals, saturate); });
  cudaFree(state);
}
                          ...
                          bool subnormals, bool saturate)
{
  ...
  mm_sr_fma_impl<SHMEM_SIZE, uint32_t><<<block_dim, thread_dim>>>(
      ...
      state,
      M, K, N,
      [man_fma, exp_fma, bias_fma, rbits_fma, subnormals, saturate] __device__(float x, uint32_t rnd)
      { return cast_fp_stochastic(x, rnd, rbits_fma, man_fma, exp_fma, bias_fma, subnormals, saturate); });
  cudaFree(state);
}
                                  ...
                                  bool subnormals, bool saturate)
{
  ...
  layernorm_forward(
      ...
      [man_mul, exp_mul, bias_mul, subnormals, saturate] __device__(float x)
      { return cast_fp_nearest_even(x, man_mul, exp_mul, bias_mul, subnormals, saturate); },
      [man_div, exp_div, bias_div, subnormals, saturate] __device__(float x)
      { return cast_fp_nearest_even(x, man_div, exp_div, bias_div, subnormals, saturate); },
      [man_sqrt, exp_sqrt, bias_sqrt, subnormals, saturate] __device__(float x)
