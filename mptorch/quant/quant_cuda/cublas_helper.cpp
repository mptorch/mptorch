...
void get_cublas_configuration(CUBLASMatrixType inp_matrix_type,
...
void float_mm_cublas(Tensor a, Tensor b, Tensor c, int M, int N, int K,
                     ...
                     CUBLASComputeType compute_type, bool pedantic) {
  // Tensors a, b, and c are assumed to have the right datatype and be properly
  // transposed.
  CUBLASGemmConfig config;
  ...
  math =
      (cublasMath_t)(math | CUBLAS_MATH_DISALLOW_REDUCED_PRECISION_REDUCTION);
  ...
  case CUDA_R_16F: {
    ...
    error =
        cublasGemmEx(get_cublas_handle(), CUBLAS_OP_N, CUBLAS_OP_N, M, N, K,
                     &alpha, a.data_ptr(), config.matrix_a, M, b.data_ptr(),
                     config.matrix_b, K, &beta, c.data_ptr(), config.matrix_c,
                     M, config.compute, CUBLAS_GEMM_DEFAULT);
  } break;
  default: {
    float alpha = 1.f;
    float beta = 0.f;
...
void float_bmm_cublas(Tensor a, Tensor b, Tensor c, int M, int N, int K,
  ...
  case CUDA_R_16F: {
    half alpha = __float2half(1.f);
    half beta = __float2half(0.f);
    error = cublasGemmBatchedEx(
        get_cublas_handle(), CUBLAS_OP_N, CUBLAS_OP_N, M, N, K, &alpha, d_dA,
        config.matrix_a, M, d_dB, config.matrix_b, K, &beta, d_dC,
        config.matrix_c, M, B, config.compute, CUBLAS_GEMM_DEFAULT);
  } break;
  default: {
    float alpha = 1.f;
    float beta = 0.f;
