...
void float_quantize_nearest_gemm(Tensor a, Tensor b, Tensor c, int M, int N,
  ...
  CHECK_INPUT(b);
  CHECK_INPUT(c);
  float_quantize_nearest_gemm_cuda(a, b, c, M, N, K, man_mul, exp_mul, man_add,
...
void float_quantize_stochastic_gemm(Tensor a, Tensor b, Tensor c, int M, int N,
  ...
  CHECK_INPUT(b);
  CHECK_INPUT(c);
  float_quantize_stochastic_gemm_cuda(a, b, c, M, N, K, man_mul, exp_mul,
...
void fixed_point_quantize_nearest_gemm(Tensor a, Tensor b, Tensor c, int M,
...
void fixed_point_quantize_nearest_gemm_fma(Tensor a, Tensor b, Tensor c, int M,
...
void fixed_point_quantize_stochastic_gemm(Tensor a, Tensor b, Tensor c, int M,
                                          int N, int K, int wl_add, int fl_add,
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("fixed_point_quantize_nearest_gemm_fma",
        &fixed_point_quantize_nearest_gemm_fma,
        "Low-Bitwidth Fixed Point Number FMA-based GEMM Quantization (CUDA)");
  m.def("fixed_point_quantize_stochastic_gemm",
        &fixed_point_quantize_stochastic_gemm,
        "Low-Bitwidth Fixed Point Number GEMM with Stochastic Quantization "
        "(CUDA)");
  m.def("fixed_point_quantize_stochastic_gemm_fma",
        &fixed_point_quantize_stochastic_gemm_fma,
