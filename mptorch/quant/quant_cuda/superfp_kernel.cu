...
__device__ float cast_superfp_nearest_even(float origin_float, int man_bits, int exp_bits, int bias,
...
__device__ float cast_superfp_nearest_away(float origin_float, int man_bits, int exp_bits, int bias,
                                           int binades_l, int binades_h, bool saturate)
{
  int32_t sat = saturate;
  uint32_t target;
...
__device__ float cast_absolute_up(float origin_float, int man_bits, int exp_bits, int bias,
                                  int binades_l, int binades_h, bool saturate)
{
...
__device__ float cast_absolute_down(float origin_float, int man_bits, int exp_bits, int bias,
                                    int binades_l, int binades_h, bool saturate)
{
  if (origin_float == 0.0f)
    return 0.0f;
...
__device__ float cast_superfp_up(float origin_float, int man_bits, int exp_bits, int bias,
                                 int binades_l, int binades_h, bool saturate)
{
  if (origin_float >= 0.0f)
...
__device__ float cast_superfp_down(float origin_float, int man_bits, int exp_bits, int bias,
                                   int binades_l, int binades_h, bool saturate)
{
  if (origin_float >= 0.0f)
    return cast_absolute_down(origin_float, man_bits, exp_bits, bias, binades_l, binades_h, saturate);
...
__device__ float cast_superfp_zero(float origin_float, int man_bits, int exp_bits, int bias,
                                   int binades_l, int binades_h, bool saturate)
{
  if (origin_float >= 0.0f)
...
void softmax_forward_superfp_nearest(float *a, float *o,
                                     ...
                                     bool saturate)
{
  softmax_forward(a, o, sizes, [man_exp, exp_exp, binades_exp_l, binades_exp_u, saturate] __device__(float x)
                  { return cast_superfp_nearest(x, man_exp, exp_exp, binades_exp_l, binades_exp_u, saturate); }, [man_off, exp_off, binades_off_l, binades_off_u, saturate] __device__(float x)
                  { return cast_superfp_nearest(x, man_off, exp_off, binades_off_l, binades_off_u, saturate); }, [man_acc, exp_acc, binades_acc_l, binades_acc_u, saturate] __device__(float x)
...
void softmax_lse_forward_superfp_nearest(float *a, float *o,
                                         ...
                                         bool saturate)
{
  softmax_lse_forward(a, o, sizes, [man_off, exp_off, binades_off_l, binades_off_u, saturate] __device__(float x)
                      { return cast_superfp_nearest(x, man_off, exp_off, binades_off_l, binades_off_u, saturate); }, [man_lse, exp_lse, binades_lse_l, binades_lse_u, saturate] __device__(float x)
...
void softmax_backward_superfp_nearest(float *a, float *g, float *o,
                                      ...
                                      bool saturate)
{
  softmax_backward(a, g, o, sizes, [man_add, exp_add, binades_add_l, binades_add_u, saturate] __device__(float x)
                   { return cast_superfp_nearest(x, man_add, exp_add, binades_add_l, binades_add_u, saturate); }, [man_mul, exp_mul, binades_mul_l, binades_mul_u, saturate] __device__(float x)
                   { return cast_superfp_nearest(x, man_mul, exp_mul, binades_mul_l, binades_mul_u, saturate); });
}
...
void layernorm_forward_superfp_nearest(float *input, float *weight, float *bias,
                                       ...
                                       bool saturate)
{
  layernorm_forward(input, weight, bias, output, mean, rstd, eps, sizes, [man_acc, exp_acc, binades_acc_l, binades_acc_u, saturate] __device__(float x)
                    { return cast_superfp_nearest(x, man_acc, exp_acc, binades_acc_l, binades_acc_u, saturate); }, [man_mul, exp_mul, binades_mul_l, binades_mul_u, saturate] __device__(float x)
                    { return cast_superfp_nearest(x, man_mul, exp_mul, binades_mul_l, binades_mul_u, saturate); }, [man_div, exp_div, binades_div_l, binades_div_u, saturate] __device__(float x)
                    { return cast_superfp_nearest(x, man_div, exp_div, binades_div_l, binades_div_u, saturate); }, [man_sqrt, exp_sqrt, binades_sqrt_l, binades_sqrt_u, saturate] __device__(float x)
                                        ...
                                        bool saturate)
{
  ...
  layernorm_backward(input, grad_output, weight, bias, mean, rstd, grad_input, grad_gamma, grad_beta, xhat_gradient, sizes, [man_acc, exp_acc, binades_acc_l, binades_acc_u, saturate] __device__(float x)
                     { return cast_superfp_nearest(x, man_acc, exp_acc, binades_acc_l, binades_acc_u, saturate); }, [man_mul, exp_mul, binades_mul_l, binades_mul_u, saturate] __device__(float x)
                     { return cast_superfp_nearest(x, man_mul, exp_mul, binades_mul_l, binades_mul_u, saturate); }, [man_div, exp_div, binades_div_l, binades_div_u, saturate] __device__(float x)
