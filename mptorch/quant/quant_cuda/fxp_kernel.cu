                    ...
                    int t_min_mul, int t_max_mul)
{
  launch_mm_addmul(
      a, b, c, M, K, N, 1, false,
      [sigma_add, t_min_add, t_max_add] __device__(float x)
      { return cast_fxp_nearest(x, sigma_add, t_min_add, t_max_add); },
      [sigma_mul, t_min_mul, t_max_mul] __device__(float x)
                        ...
                        int sigma_fma, int t_min_fma, int t_max_fma)
{
  launch_mm_fma(
      a, b, c, M, K, N, 1, false,
      [sigma_fma, t_min_fma, t_max_fma] __device__(float x)
...
{
  launch_mm_stochastic<float>(
      a, b, c, M, K, N, 1,
      [sigma_add, t_min_add, t_max_add] __device__(float x, float rand_prob)
      { return cast_fxp_stochastic(x, rand_prob, sigma_add, t_min_add, t_max_add); },
      [sigma_mul, t_min_mul, t_max_mul] __device__(float x, float rand_prob)
      { return cast_fxp_stochastic(x, rand_prob, sigma_mul, t_min_mul, t_max_mul); });
}
...
void mm_fxp_fma_stochastic(float *a, float *b, float *c, int M, int K, int N,
                           int sigma_fma, int t_min_fma, int t_max_fma)
{
  launch_mm_fma_stochastic<float>(
      a, b, c, M, K, N, 1,
      [sigma_fma, t_min_fma, t_max_fma] __device__(float x, float rand_prob)
      { return cast_fxp_stochastic(x, rand_prob, sigma_fma, t_min_fma, t_max_fma); });
}
