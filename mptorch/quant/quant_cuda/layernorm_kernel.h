...
__global__ void layernorm_forward_impl(
    const float *__restrict__ in_arr, const float *__restrict__ w_array,
    const float *__restrict__ b_array, float *out_arr, float *m_array,
    float *r_array, float eps, const DimSizes sizes, Qacc quant_acc,
    Qmul quant_mul, Qdiv quant_div, Qsqrt quant_sqrt) {
  extern __shared__ float shared[];

  int tid = threadIdx.x;
  int warp =
      threadIdx.x /
      warpSize; // groups of 32 threads (which warp the thread belongs to)
  int lane = threadIdx.x %
             warpSize; // a warp has 32 lanes (id of the thread in a warp)
  ...
  // compute variance by reducing sum of elements then dividing by size of
  // channel done similarly to mean
  float v_sum = 0;
  for (int k = tid; k < sizes.channel; k += blockDim.x) {
    int idx = k * sizes.inner;
...
// saves some values for later second pass to calculate gradient of weights and
// biases
template <class Qacc, class Qmul, class Qdiv>
__global__ void layernorm_backward_first_pass_impl(
    const float *__restrict__ in_arr, const float *__restrict__ out_grad,
    ...
    float *out_arr, float *xhat_gradient, const DimSizes sizes, Qacc quant_acc,
    Qmul quant_mul, Qdiv quant_div) {
  extern __shared__ float shared[];
  ...
  // grad_sum calculated by reducing similar to mean and variance from forward
  // pass
  for (int offset = warpSize / 2; offset > 0; offset /= 2) {
    grad_sum =
        quant_acc(grad_sum + __shfl_down_sync(0xffffffff, grad_sum, offset));
  }
  if (lane == 0) {
  ...
  // grad_sum_xhat
  for (int offset = warpSize / 2; offset > 0; offset /= 2) {
    grad_sum_xhat = quant_acc(
        grad_sum_xhat + __shfl_down_sync(0xffffffff, grad_sum_xhat, offset));
  }
  if (lane == 0) {
...
// uses output gradient from forward pass and xhat_gradient from first backward
// pass used to calculate grad_gamma (gradient of weights) and grad_beta
// (gradient of biases)
template <class Qacc>
__global__ void layernorm_backward_second_pass_impl(
    const float *__restrict__ xhat_gradient, const float *__restrict__ out_grad,
    float *grad_gamma, float *grad_beta, const DimSizes sizes, Qacc quant_acc) {
...
template <class Qacc, class Qmul, class Qdiv, class Qsqrt>
void layernorm_forward(const float *in_arr, const float *w_array,
                       const float *b_array, float *out_arr, float *m_array,
                       float *r_array, float eps, const DimSizes &sizes,
                       Qacc quant_acc, Qmul quant_mul, Qdiv quant_div,
                       Qsqrt quant_sqrt) {
  int blocks = sizes.outer * sizes.inner;
  int block_size = 64;
  // printf("inner: %d, outer: %d, channel: %d", sizes.inner, sizes.outer,
  // sizes.channel);
...
void layernorm_backward(const float *in_arr, const float *out_grad,
                        ...
                        float *xhat_gradient, const DimSizes &sizes,
                        Qacc quant_acc, Qmul quant_mul, Qdiv quant_div) {
  int blocks = sizes.outer * sizes.inner;
  int block_size = 64;
  size_t shared_mem_size = (block_size / 32) * sizeof(float);
  layernorm_backward_first_pass_impl<<<blocks, block_size, shared_mem_size>>>(
      in_arr, out_grad, w_array, b_array, m_array, r_array, grad_input,
      xhat_gradient, sizes, quant_acc, quant_mul, quant_div);
  blocks = sizes.channel / block_size + (sizes.channel % block_size != 0);
  layernorm_backward_second_pass_impl<<<blocks, block_size>>>(
      xhat_gradient, out_grad, grad_gamma, grad_beta, sizes, quant_acc);
}