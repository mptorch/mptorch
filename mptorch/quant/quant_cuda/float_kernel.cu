...
__device__ float cast_fp_stochastic(float origin_float, 
      unsigned int rand_prob, int man_bits, int exp_bits,
      bool subnormal_support = true) {
  unsigned int target, quantize_bits;
  target = FLOAT_TO_BITS(&origin_float);
  float quantized;
...
__global__ void float_kernel_stochastic(float *__restrict__ a,
  ...
  if (index < size)
    o[index] = cast_fp_stochastic(a[index], (unsigned int)r[index], 
                                  man_bits, exp_bits, subnormal_support);
}
  ...
  // load in elements for this tile
    ...
    for (int j = 0; j < blockDim.x; j++) {
      float y = cast_fp_nearest(cast_fp_nearest(s_a[ty * blockDim.x + j] * 
                  s_b[j * blockDim.x + tx], man_mul, exp_mul, subnormal_support) - comp,
                  man_add, exp_add, subnormal_support);
      float t = cast_fp_nearest(sum + y, man_add, exp_add, subnormal_support);
      comp = cast_fp_nearest(cast_fp_nearest(t - sum, man_add, exp_add, subnormal_support) - y,
                  man_add, exp_add, subnormal_support);
      sum = t;
    }
...
__global__ void gemm_fp_fma_nearest(float *__restrict__ a, float *__restrict__ b,
                                  float *__restrict__ c, int M, int K, int N,
  ...
  // load in elements for this tile
    ...
    // do matrix multiplication on the small matrices
    for (int j = 0; j < blockDim.x; j++) {
      tmp = cast_fp_nearest(fmaf(s_a[ty * blockDim.x + j], s_b[j * blockDim.x + tx], tmp),
...
__global__ void gemm_fp_stochastic(float *__restrict__ a, float *__restrict__ b,
  ...
  // load in elements for this tile
    ...
    for (int j = 0; j < blockDim.x; j++) {
      ...
      tmp = cast_fp_stochastic(tmp + cast_fp_stochastic(s_a[ty * blockDim.x + j] * 
                  s_b[j * blockDim.x + tx], rmul, man_mul, exp_mul, subnormal_support), 
...
__global__ void gemm_fp_fma_stochastic(float *__restrict__ a, float *__restrict__ b,
                                  float *__restrict__ c,
                                  curandState_t *state, // int *__restrict__ r,
                                  int M, int K, int N,
  ...
  // load in elements for this tile
    ...
    for (int j = 0; j < blockDim.x; j++) {
      ...
      tmp = cast_fp_stochastic(fmaf(s_a[ty * blockDim.x + j], s_b[j * blockDim.x + tx], tmp),
                  rfma, man_fma, exp_fma, subnormal_support);
    }
