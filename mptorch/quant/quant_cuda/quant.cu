...
void float_quantize_gemm_cuda(Tensor a, Tensor b, Tensor c, int M, int N, int K, 
  ...
  //tvm_gemm_fp_fxp<<<grid_size, block_size>>>(a.data<float>(), b.data<float>(), c.data<float>(),
  //          M, K, N, man_bits, exp_bits, 6, 6, false);
  //tvm_gemm_fma<<<grid_size, block_size>>>(a.data<float>(), b.data<float>(), c.data<float>(),
  //          M, K, N, man_bits, exp_bits);
  return;
}