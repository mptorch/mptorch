...
template <class T, class F> __device__ T warp_reduce(T acc, F fn) {
...
__global__ void softmax_forward_impl(const float *__restrict__ input_array,
                                     float *output_array, const DimSizes sizes,
                                     Qexp quant_exp, Qoff quant_off,
                                     Qacc quant_acc) {
  extern __shared__ float shared[];

  int tid = threadIdx.x;
  int warp = threadIdx.x /
             warpSize; // which warp within this block the thread belongs to
  ...
  // each thread computes part of the maximum by iterating over its
  // corresponding position along the row, as many times as required to cover
  // all the row
  float max = -INFINITY;
  ...
  // each thread now contains part of the maximum of the row, we combine these
  // maximums into a per-warp maximum, stored in the 0th thread of the warp
  // (lane = 0)
  ...
  __syncthreads();
  // reduce the maximum of each warp into the 0th thread of the block (warp 0,
  // lane 0)
  if (tid == 0) {
  ...
  // compute the sum of exp(x[i] - max), using a similar approach as for the
  // maximum, but reducing a sum instead

  float sum = 0.f;
  for (int k = tid; k < sizes.channel; k += blockDim.x) {
    int idx = k * sizes.inner;
    sum = quant_acc(sum +
                    output[idx]); // sum the previously computed exponentials
  }
  auto qadd = [&quant_acc] __device__(float a, float b) {
    return quant_acc(a + b);
  };
...
__global__ void softmax_lse_forward_impl(const float *__restrict__ input_array,
                                         float *output_array,
                                         const DimSizes sizes, Qoff quant_off,
                                         Qlse quant_lse) {
  extern __shared__ float shared[];
  ...
  for (int k = tid; k < sizes.channel; k += blockDim.x) {
    int idx = k * sizes.inner;
    output[idx] = expf(
        quant_off(output[idx] - lgs)); // quantization handled by output_quant
  }
}

template <class Qadd, class Qmul>
__global__ void softmax_backward_impl(const float *__restrict__ input_array,
                                      const float *__restrict__ out_gradient,
                                      float *output_array, const DimSizes sizes,
  ...
  auto qadd = [&quant_add] __device__(float a, float b) {
    return quant_add(a + b);
  };
  ...
  __syncthreads();
  if (tid == 0) {
    shared_weighted_grad_sum[0] =
        shared_reduce(qadd, shared_weighted_grad_sum, warpsPerBlock);
  }
  __syncthreads();
  weighted_grad_sum = shared_weighted_grad_sum[0];

  // Last step, subtract the weighted sum from the gradient, and divide by input
  // sum
  for (int k = tid; k < sizes.channel; k += blockDim.x) {
    int idx = k * sizes.inner;
...
template <class Qexp, class Qoff, class Qacc>
void softmax_forward(const float *input_array, float *output_array,
                     const DimSizes &sizes, Qexp quant_exp, Qoff quant_off,
                     Qacc quant_acc) {
...
template <class Qoff, class Qlse>
void softmax_lse_forward(const float *input_array, float *output_array,
                         const DimSizes &sizes, Qoff quant_off,
                         Qlse quant_lse) {
  int blocks = sizes.outer * sizes.inner;
  int block_size = 64;
...
template <class Qadd, class Qmul>
void softmax_backward(const float *input_array, const float *out_gradient,
                      float *output_array, const DimSizes &sizes,
                      Qadd quant_add, Qmul quant_mul) {
  int blocks = sizes.outer * sizes.inner;
  int block_size = 64;
